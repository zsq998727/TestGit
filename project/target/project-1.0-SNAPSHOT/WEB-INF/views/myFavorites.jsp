...
<script type="text/javascript">
    ...
    function build_img(result) {
        ...
        $.each(food, function (index, item) {
            ...
            var btnTd = $("<p>").addClass("text-center").append(delBtn);

            var div = $("<div></div>").addClass("col-lg-3 col-md-4 col-sm-6").append($("<a></a>").attr("href", "details?id=" + item.dishName.id).append
            ($("<div></div>").addClass("thumbnail").append($("<img>").attr("src", item.dishName.img)).append($("<div></div>").addClass("caption").append
            ($("<p>").addClass("text-center").append(item.dishName.name))))).append(btnTd);
            $("#infor").append(div);
        });
    }
    ...
    $(document).on("click", ".delete_btn", function () {
        ...
        if (confirm("确认删除【" + dishName + "】吗？")) {
            $.ajax({
                url: "deleteFaFood",
                type: "post",
                data: {fid: dishId, userid:${userInfo.id}},
                success: function (result) {
                    alert(result.msg)
                    to_page(currentPage);
                }
